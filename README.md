...
- **Главная** `/` рассказывает о сервисе: три раздела, как это работает, почему без сервера, все конвертеры, форматы и вопросы. Самого конвертера на главной нет — кнопки ведут в нужный раздел.
- **Картинки** `/konverter-izobrazhenij/` — конвертер изображений и инструкция, как с ним работать.
- **Документы** `/dokumenty/` — конвертер PDF, Word, Excel, CSV, TXT и Markdown.
- **Инструменты** `/instrumenty/` — сжать фото, сжать до N КБ, изменить размер.
- Плюс посадочные страницы под частые запросы («HEIC в JPG», «Объединить PDF» и т. д.), справочник форматов, инструкция и правовые страницы.

<p align="center">
  <img src="docs/screenshots/home-sections.png" alt="Главная: три раздела — картинки, документы, инструменты" width="900">
</p>
...
Кроме главной и разделов, у сайта десятки посадочных страниц под частые запросы. На каждой свой текст, вопросы и ответы, а конвертер открывается уже настроенным под задачу.
...
|---|---|
| `/` | онлайн-конвертер картинок и документов (обзор сервиса) |
| `/konverter-izobrazhenij/` | конвертер изображений онлайн |
| `/dokumenty/` | конвертер документов онлайн |
| `/png-v-jpg/` | png в jpg |
| `/jpg-v-png/` | jpg в png |
